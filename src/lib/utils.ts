...
export const MILESTONES = [
  ...
  { week: 12, label: 'First trimester ends', desc: 'Risk of miscarriage decreases significantly' },
  { week: 16, label: 'Fetal movement begins', desc: 'You may feel first kicks (quickening)' },
  { week: 20, label: 'Anatomy scan', desc: "Detailed ultrasound checks baby's organs" },
  { week: 24, label: 'Viability milestone', desc: 'Baby could survive with medical support' },
  { week: 28, label: '3rd trimester begins', desc: 'Rapid brain & weight development' },
...
export const NUTRITION_DATA: Record<string, {
  ...
  '1st': {
    safe: ['Leafy greens 🥬', 'Fortified cereals 🌾', 'Beans & lentils 🫘', 'Eggs 🥚', 'Yogurt 🥛', 'Citrus fruits 🍊'],
    avoid: ['Raw fish/sushi 🍣', 'Deli meats 🥩', 'Unpasteurized cheese 🧀', 'Alcohol 🚫', 'High-mercury fish 🐟', 'Raw eggs 🥚'],
    supps: ['Folic Acid 400mcg', 'Vitamin D 1000IU', 'Iron 27mg', 'Prenatal Multivitamin'],
    nutrients: [
  ...
  '2nd': {
    safe: ['Iron-rich red meat 🥩', 'Salmon (cooked) 🐟', 'Sweet potato 🍠', 'Broccoli 🥦', 'Nuts & seeds 🌰', 'Whole grains 🌾'],
    avoid: ['Raw sprouts 🌱', 'Licorice 🍬', 'Excess caffeine ☕', 'Processed foods 🍔', 'Raw shellfish 🦐', 'Unpasteurized juice 🧃'],
    supps: ['Iron 27mg', 'DHA/Omega-3 200mg', 'Calcium 1000mg', 'Vitamin C 85mg'],
    nutrients: [
  ...
  '3rd': {
    safe: ['High-protein foods 🥩', 'Avocado 🥑', 'Dates 🌴', 'Bananas 🍌', 'Brown rice 🍚', 'Coconut water 🥥'],
    avoid: ['Spicy foods 🌶️', 'Carbonated drinks 🥤', 'Large portions 🍽️', 'Alcohol 🚫', 'Excess salt 🧂', 'Grapefruit 🍊'],
...
export const VACCINES = [
  { id: 'tt1', name: 'Tetanus Toxoid (TT) 1st dose', when: 'As early as possible (Week 4–8)', eligibleFromWeek: 4 },
  { id: 'tt2', name: 'Tetanus Toxoid (TT) 2nd dose', when: '4 weeks after 1st dose', eligibleFromWeek: 8 },
  { id: 'flu', name: 'Influenza (Flu) Vaccine', when: 'Any trimester (recommended)', eligibleFromWeek: 1 },
  { id: 'tdap', name: 'Tdap (Whooping Cough)', when: 'Week 27–36', eligibleFromWeek: 27 },
  { id: 'hepb', name: 'Hepatitis B', when: 'If not previously vaccinated', eligibleFromWeek: 1 },
  { id: 'covid', name: 'COVID-19 Booster', when: 'Recommended — consult doctor', eligibleFromWeek: 1 },
];

export function getVaccineDefaultStatus(vaccineId: string, currentWeek: number): 'done' | 'due' | 'upcoming' {
  const vaccine = VACCINES.find(v => v.id === vaccineId);
  if (!vaccine) return 'upcoming';
  if (vaccineId === 'tt1' || vaccineId === 'tt2') return 'done';
  if (currentWeek >= vaccine.eligibleFromWeek) return 'due';
  return 'upcoming';
}
...
export const EMERGENCY_CONTACTS = [
  { icon: '🏥', number: '999 / 119', desc: 'National Emergency & Ambulance (Bangladesh)', href: 'tel:999' },
  { icon: '👩‍⚕️', number: '16000', desc: 'National Health Helpline', href: 'tel:16000' },
  { icon: '🤱', number: '10921', desc: 'Maternal & Child Health Support Line', href: 'tel:10921' },
  { icon: '🚑', number: '01700-000000', desc: "Doctor's Personal Contact", href: 'tel:01700000000' },
];
